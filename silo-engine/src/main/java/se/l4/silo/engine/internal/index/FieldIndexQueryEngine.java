...
/**
 * {@link QueryEngine} that creates a queryable index similar in functionality
 * to traditional databases.
 */
public class FieldIndexQueryEngine<T>
	implements QueryEngine<T, FieldIndexQuery<T>>
{
	private static final Logger logger = LoggerFactory.getLogger(FieldIndexQueryEngine.class);
	...
	public FieldIndexQueryEngine(
		...
		String name,
		String uniqueName,
		ListIterable<FieldDefinition<T>> fields,
		ListIterable<FieldDefinition<T>> sortFields
	)
	{
		this.name = name;
		...
		this.fields = fields.toArray(new FieldDefinition[fields.size()]);
		this.sortFields = sortFields.toArray(new FieldDefinition[sortFields.size()]);
	...
	private FieldIndexResult<T> runQuery(QueryEncounter<? extends FieldIndexQuery<T>, T> encounter)
	{
		FieldIndexQuery<T> query = encounter.getQuery();
		...
		return new FieldIndexResultImpl<>(items, total.sum(), offset, limit);
	}

	@Override
	public Mono<? extends FetchResult<?>> fetch(
		QueryEncounter<? extends FieldIndexQuery<T>, T> encounter
	)
	{
